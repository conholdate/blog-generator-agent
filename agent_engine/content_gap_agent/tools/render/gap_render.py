...
def _sentence_case_topic(topic: str, platform_terms: set[str]) -> str:
    protected = set(_UPPERCASE_TERMS)
    protected.update(t.lower() for t in platform_terms if t)
    rendered = _uppercase_protected_terms(topic, protected)
    words = rendered.split()
    if not words:
        return rendered
    first = words[0]
    if any(ch.isalpha() for ch in first) and not first.isupper():
...
def _title_case_document_title(title: str, platform_terms: set[str]) -> str:
    ...
    for i, raw in enumerate(words):
        ...
        out.append(raw.replace(core, cased, 1))
    protected = set(_UPPERCASE_TERMS)
    protected.update(t.lower() for t in platform_terms if t)
    return _uppercase_protected_terms(" ".join(out), protected)
...
def render_gaps_md(title: str, coverage_json: Dict[str, Any]) -> str:
    ...
    for gr in quick_wins[:8]:
        topic = _sentence_case_topic(gr.topic, platform_terms)
        src = f"{baseline.upper()} coverage: {topic}" if baseline else f"Well-covered topic: {topic}"
        tgt = f"New guides for: {', '.join(p.upper() for p in gr.missing_platforms[:4])}" + (
            "..." if len(gr.missing_platforms) > 4 else ""
        )
        anchor = (
            f"{topic} in {', '.join(p.upper() for p in gr.missing_platforms[:2])}"
            if gr.missing_platforms
            else topic
        )
    ...
    if not high_priority:
        ...
        for i, (cat, sub, gr) in enumerate(high_priority, start=1):
            missing = ", ".join(p.upper() for p in gr.missing_platforms[:6]) + ("..." if len(gr.missing_platforms) > 6 else "")
            topic = _sentence_case_topic(gr.topic, platform_terms)
            suggested_titles = "; ".join([f"{topic} — {p.upper()}" for p in gr.missing_platforms[:3]])
            hp_rows.append([str(i), f"**{cat} / {sub}**", topic, missing, suggested_titles])
    ...
    for p in platforms:
        ...
        missing = platform_missing_counts.get(p, 0)
        pct = (covered / total_rows * 100.0) if total_rows else 0.0
        plat_rows.append([f"**{p.upper()}**", str(covered), f"{pct:.1f}%", str(missing)])
    ...
    for (cat, sub), items in ranked_clusters[:12]:
        ...
        top_missing = ", ".join([p.upper() for p, _ in miss_counter.most_common(4)])
    ...
    if not quick_wins:
        ...
        for gr in quick_wins:
            ...
            qw_rows.append([_sentence_case_topic(gr.topic, platform_terms), missing, effort])

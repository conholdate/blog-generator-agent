...
def write_action_items(result: AuditResult, path: Path, detailed_outputs: bool = False, run_context: dict[str, object] | None = None) -> None:
    ...
    lines = [
        ...
        *markdown_table(["Area", "Action Items"], [[area, str(count)] for area, count in area_counts.most_common()]),
        ...
        "",
        *grouped_action_item_sections(items, result, detailed_outputs),
        "",
    ]
    if llm_rows:
        lines += [
...
def grouped_action_item_sections(items: list[dict[str, str]], result: AuditResult, include_source: bool = False) -> list[str]:
    ...
    for file_path, group in post_groups.items():
        ...
        lines += [
            ...
            f"File: `{file_path}`",
            "",
            *action_item_table_without_file(group, "Post-level", include_source),
            "",
        ]
    ...
    for file_path, group in sitewide_groups.items():
        lines += [
            ...
            f"Scope: `{file_path}`",
            "",
            *action_item_table_without_file(group, "Sitewide", include_source),
            "",
        ]
...
def action_item_table_without_file(items: list[dict[str, str]], blank_line_scope: str, include_source: bool = False) -> list[str]:
    headers = ["ID", "Priority", "Severity", "Area", "Issue", "Line / Scope", "Post Priority", "Recommended Action", "Effort", "Impact"]
    if include_source:
...
def write_draft_fixes(result: AuditResult, draft_dir: Path, max_fixes: int | None, priority_only: bool) -> None:
    ...
    for post in posts:
        ...
        fixed, applied = apply_draft_issue_fixes(original, post)
        appendix = draft_fix_appendix(post)
        notes = [
            "<!--",
            ...
            f"Applied fixes: {'; '.join(applied) if applied else 'None'}",
            "-->",
            "",
        ]
        target.write_text("\n".join(notes) + fixed.rstrip() + appendix + "\n", encoding="utf-8")
...
def apply_draft_issue_fixes(text: str, post: Post) -> tuple[str, list[str]]:
    fixed = text
    applied: list[str] = []
    for issue in post.issues:
        if issue.issue_type == "unresolved_api_module":
            fixed, note = apply_unresolved_api_module_fix(fixed, post, issue)
            if note:
                applied.append(note)
        elif issue.issue_type in {"deprecated_api_symbol", "unresolved_api_symbol", "unresolved_api_class", "unresolved_api_member"}:
            fixed, note = apply_backticked_replacement_fix(fixed, issue)
            if note:
                applied.append(note)
        elif issue.issue_type == "missing_title":
            fixed, note = apply_front_matter_scalar_fix(fixed, "title", _suggest_title(post))
            if note:
                applied.append(note)
        elif issue.issue_type == "missing_description":
            fixed, note = apply_front_matter_scalar_fix(fixed, "description", _suggest_description(post))
            if note:
                applied.append(note)
    return fixed, applied


def apply_unresolved_api_module_fix(text: str, post: Post, issue: Issue) -> tuple[str, str]:
    invalid = unresolved_module_from_issue(issue)
    replacement = replacement_namespace_from_issue(post, issue, invalid)
    if not invalid or not replacement or invalid == replacement:
        return text, ""
    updated = replace_import_module(text, invalid, replacement)
    if updated == text:
        return text, ""
    return updated, f"unresolved_api_module: {invalid} -> {replacement}"


def unresolved_module_from_issue(issue: Issue) -> str:
    match = re.search(r"imports\s+`([^`]+)`", issue.explanation)
    if match:
        return match.group(1)
    quoted = re.findall(r"`([^`]+)`", issue.recommended_fix)
    return quoted[0] if quoted else ""


def replacement_namespace_from_issue(post: Post, issue: Issue, invalid: str) -> str:
    candidates = [candidate for candidate in re.findall(r"`([^`]+)`", issue.recommended_fix) if candidate != invalid]
    candidate = preferred_namespace_candidate(candidates)
    if not candidate:
        return ""
    if candidate == "aspose.barcode":
        return refine_aspose_barcode_namespace(candidate, post)
    return candidate


def preferred_namespace_candidate(candidates: list[str]) -> str:
    if not candidates:
        return ""
    lowercase_aspose = [item for item in candidates if item == item.lower() and item.startswith("aspose.")]
    if lowercase_aspose:
        return lowercase_aspose[0]
    non_java = [item for item in candidates if not item.lower().startswith("com.aspose.")]
    return non_java[0] if non_java else candidates[0]


def refine_aspose_barcode_namespace(namespace: str, post: Post) -> str:
    title_description = f"{post.title}\n{post.description}".lower()
    body = post.body.lower()
    code = "\n".join(sample.code for sample in post.code_samples).lower()
    generation_score = namespace_intent_score(
        title_description,
        body,
        code,
        [r"\b(generate|generator|create|encode)\b"],
        [r"barcodegenerator", r"encodetypes", r"barcodeimageformat"],
    )
    recognition_score = namespace_intent_score(
        title_description,
        body,
        code,
        [r"\b(read|reader|recognize|recognition|scan|decode)\b"],
        [r"barcodereader", r"decodetype", r"qualitysettings"],
    )
    if recognition_score > generation_score:
        return f"{namespace}.barcoderecognition"
    if generation_score > recognition_score:
        return f"{namespace}.generation"
    return namespace


def namespace_intent_score(title_description: str, body: str, code: str, prose_patterns: list[str], code_patterns: list[str]) -> int:
    score = 0
    for pattern in prose_patterns:
        if re.search(pattern, title_description):
            score += 3
...
def replace_import_module(text: str, invalid: str, replacement: str) -> str:
    module = re.escape(invalid)
    updated = re.sub(
        rf"^(\s*from\s+){module}(\s+import\s+)",
        rf"\g<1>{replacement}\g<2>",
        text,
        flags=re.M,
    )
    updated = re.sub(
        rf"^(\s*import\s+){module}(\s+as\s+[A-Za-z_][A-Za-z0-9_]*)?(\s*(?:#.*)?$)",
        rf"\g<1>{replacement}\g<2>\g<3>",
        updated,
        flags=re.M,
    )
    return updated
...
def apply_backticked_replacement_fix(text: str, issue: Issue) -> tuple[str, str]:
    if not issue.recommended_fix.lower().startswith("replace "):
        return text, ""
    quoted = re.findall(r"`([^`]+)`", issue.recommended_fix)
    if len(quoted) < 2:
        return text, ""
    ...
    if not old or not new or old == new:
        return text, ""
    updated = re.sub(rf"(?<![A-Za-z0-9_]){re.escape(old)}(?![A-Za-z0-9_])", new, text)
    if updated == text:
        return text, ""
    return updated, f"{issue.issue_type}: {old} -> {new}"


def apply_front_matter_scalar_fix(text: str, key: str, value: str) -> tuple[str, str]:
    if not value:
        return text, ""
    if text.startswith("---\n"):
        updated = upsert_yaml_front_matter_scalar(text, key, value)
    elif text.startswith("+++\n"):
        updated = upsert_toml_front_matter_scalar(text, key, value)
    else:
        updated = f"---\n{key}: {yaml_quote(value)}\n---\n\n{text}"
    if updated == text:
        return text, ""
    return updated, f"{key}: added"


def upsert_yaml_front_matter_scalar(text: str, key: str, value: str) -> str:
    end = text.find("\n---", 4)
    if end == -1:
        return text
    raw = text[4:end]
    ...
    for idx, line in enumerate(lines):
        if key_re.match(line):
            ...
            return "---\n" + "\n".join(lines) + "\n---\n" + body
    insert_at = 1 if key != "title" and lines and re.match(r"^title\s*:", lines[0]) else 0
    lines.insert(insert_at, replacement)
    return "---\n" + "\n".join(lines) + "\n---\n" + body


def upsert_toml_front_matter_scalar(text: str, key: str, value: str) -> str:
    end = text.find("\n+++", 4)
    if end == -1:
        return text
    raw = text[4:end]
...
def draft_fix_appendix(post: Post) -> str:
    issue_types = {issue.issue_type for issue in post.issues}
    sections: list[str] = []
    if "missing_faq" in issue_types:
        sections.append(
            "## Suggested FAQ\n\n"

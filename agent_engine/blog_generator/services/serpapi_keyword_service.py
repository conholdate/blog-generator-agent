...
class SerpAPIKeywordService:
    ...
    async def fetch_keywords(self, topic: str, product_name: str = None, max_keywords: int = None) -> Dict:
        ...
        try:
            ...
            # Apply max_keywords limit by distributing evenly across categories
            if max_keywords is not None and max_keywords > 0:
                total = len(primary) + len(secondary) + len(long_tail)
                if total > max_keywords:
                    # Distribute budget proportionally across categories
                    total_raw = max(total, 1)
                    primary_count   = round(max_keywords * len(primary)   / total_raw)
                    secondary_count = round(max_keywords * len(secondary) / total_raw)
                    long_tail_count = max_keywords - primary_count - secondary_count

                    primary   = primary[:max(primary_count, 0)]
                    secondary = secondary[:max(secondary_count, 0)]
                    long_tail = long_tail[:max(long_tail_count, 0)]

...
class BlogOrchestrator: 
    ...
    async def create_blog_autonomously(
        ...
        try:
            ...
            # Retrieved-snippet posts only (no-op for LLM-generated code):
            # drop secondary code blocks that reference an unverified API, then
            # link the source Example-Agent repo.
            result.final_output = strip_unverified_code_blocks(result.final_output, generated_code)
            ...
            # Drop the internal <!--[CODE_SNIPPET_*]--> scaffolding now that every
            # marker-dependent step has run.
            final_content = strip_snippet_markers(final_content)
